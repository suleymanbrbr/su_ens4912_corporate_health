...
class SUT_RAG_Engine:
    ...
    def _remove_strikethrough_and_save_temp(self, input_path: str) -> str:
        ...
        print(f"[PREP] Removing strikethrough text from '{input_path}'...")
        doc = Document(input_path)
        temp_output_path = "temp_cleaned_sut.docx"
        # Iterate through paragraphs and runs to remove strikethrough text
        for paragraph in doc.paragraphs:
            for run in paragraph.runs:
                if run.font.strike:
                    run.text = ''
        
        # Also iterate through tables for the same purpose
    ...
    def _get_markdown_chunks(self, md_path) -> List:
        ...
        processed_text = header_pattern.sub(replace_with_markdown_header, markdown_text)
...
# In sut_rag_core.py
    ...
    def get_section_by_title(self, section_title: str) -> str:
        """
        Retrieves the content of a section by finding the BEST fuzzy match for its title,
        ...
        # A function to clean the strings by removing punctuation and making them lowercase
        ...
        for chunk_id, metadata_json in all_metadata:
            try:
                ...
                # Compare the cleaned versions of the strings
        ...
        # Lowered the threshold slightly to be more forgiving, but the cleaning should make it 100
    ...
    def search_for_related_sections(self, query: str) -> str:
        """
        Performs a full-text keyword search across the entire database to find
        chunks containing the query. Returns a summary of findings.
        """
